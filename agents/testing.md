...
**Mode:** regression-driven, coverage-aware.
**Objective:** write comprehensive, meaningful tests that catch real bugs — not tests
that just inflate coverage numbers.

**Tests protect against regressions. If a test wouldn't catch a real bug, don't write it.**

**Scope:** if implementation used TDD, unit tests for pure logic already exist. Your
job shifts to **test expansion** — integration, E2E, security, edge cases, coverage
gaps. Do NOT rewrite existing passing tests. Build on them.
...
The updated state MUST validate against
[`pipeline-state.schema.json`](../pipeline-state.schema.json). Set
`phases.testing.status` according to the table below:

| Outcome | `phases.testing.status` | Triggers |
|---------|-------------------------|----------|
| All tests pass, coverage ≥ target | `completed` | Advance to next selected phase (typically **documentation**) |
| Tests still failing, fix-test-plan written | `blocked` | Orchestrator routes back to **implementation** (fix-test mode) |
| Generating tests, batch in progress | `in-progress` | Resume from `test_batches_completed` |

Never set `status: "completed"` while `test_counts.failing > 0`. Never set
`status: "blocked"` without producing `fix-test-plan.md`.
...
```json
{
  "testing": {
    ...
    "test_counts": { "unit": "N", "integration": "N", "e2e": "N", "passing": "N", "failing": 0 },
    "coverage": { "lines": "N", "statements": "N", "functions": "N", "branches": "N" },
...
- Switch branches silently — testing should stay on the implementation branch.

You may run `git stash` to temporarily set aside edits while investigating a
flaky test, but restore the stash before handing off.

**Resume safety (idempotency):**
- Read `pipeline-state.json` first. If `phases.testing.status` is `in-progress`,
  resume from `test_batches_completed` — do not re-emit tests you have already
  written.
- In **re-test mode** (invoked after implementation fixed a `blocked` verdict),
  run the full suite first and only generate new tests for gaps that the fix
  exposed. Do not rewrite existing passing tests.
- Never overwrite an existing test file silently. If a new test case would
  collide with an existing file, read it first, add to it, and preserve
  existing tests.
- Every update to `pipeline-state.json` MUST validate against
  [`../pipeline-state.schema.json`](../pipeline-state.schema.json). Preserve
  fields owned by other phases. Status invariant: never `completed` while
  `test_counts.failing > 0`.
