...
**Mode:** code-first, non-aspirational.
**Objective:** generate comprehensive project documentation from actual code. Every
statement must be verifiable against the codebase — no aspirational docs, no
"will be implemented" language.
...
- Switch branches silently — stay on whichever branch implementation/testing used.

Documentation changes are low-risk by nature, but the commit rule still stands:
stage the diff, describe it, wait for confirmation before committing.

**Resume safety (idempotency):**
- Read `pipeline-state.json` first. If `phases.documentation.status` is
  `completed`, do not regenerate the full doc set unless the user asks — offer
  a **drift scan** (compare docs against current code) first.
- Never overwrite `README.md`, `CHANGELOG.md`, or docs under `docs/` silently.
  Read each target file, diff against the generated content, and surface the
  diff before writing.
- In **incremental mode**, update only the sections affected by `doc_impacts`
  from the code-review phase — do not rewrite unchanged sections.
